...
int main (int argc, char * argv[]) {
	...
	TIMERSTART(malloc)
	index_t num_entries = 2345000; // 1UL << 21;  // entries in DB or stream
	index_t num_features = 825;                   // length of entries ################################## maximum lenght of S or Q
	index_t num_gpus = 1;                          // number of GPUs to be used
	index_t num_streams = 8;                       // number of streams per GPU
	index_t batch_size = 1UL << 19;                // size of a batch
...
// 	assert(num_queries == 1);
	...
	load_binary(data_cpu, 1407000000, "data/seq.bin");
	load_binary(query_cpu, 825, "data/trg.bin");
...
//            for (index_t j =4845; j < 5100; j++) {
	...
	for (index_t batch = 0; /*no a priori bound check possible*/ ; batch++) {
		...
		const index_t multiplicator = 600;
